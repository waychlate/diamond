...
def evaluate_diamond_ttc(
    ...
    and evaluates step-by-step MSE over 1+ lookahead steps into the future.
    """
    ...
    for ep_idx in range(num_episodes):
        ...
        print(f"  -> Rolling out {rollout_steps} lookahead steps into future dream from primed state...")
        ...
        for step in range(rollout_steps):
            ...
            # Predict TTC from latent representation using primed LSTM memory
            if mode == "latent" and ttc_predictor is not None:
        ...
        # Plot full episode trajectory: Past Context + Future Lookahead Dream
    ...
    with open(horizon_csv_path, mode="w", newline="") as f:
        fieldnames = ["lookahead_step", "mean_mse", "std_mse", "sem_mse", "mean_mae", "std_mae", "mean_rmse"]
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        for step in range(rollout_steps):
            errs = step_maes[step]
    ...
    if len(all_records) > 0:
        ...
        print(f"Overall MSE:           {overall_mse:.4f} s²")
    ...
    # 5. Plot Average of 10 Episodes MSE vs Lookahead Step (Primary Plot requested by User)
    if any(len(errs) > 0 for errs in step_mses):
        ...
        # Plot 1: Clean Average MSE with Confidence Band and Individual Episode Lines
        plt.figure(figsize=(10, 5.5), dpi=300)
        
        # Draw individual episode curves faintly
        for ep_i in range(len(all_gt)):
            ep_sq_errors = [(all_pred[ep_i][s] - all_gt[ep_i][s]) ** 2 for s in range(rollout_steps)]
            plt.plot(lookahead_steps, ep_sq_errors, color="slategray", alpha=0.25, linewidth=1.2,
                     label="Individual Episode MSE" if ep_i == 0 else "")
            
        # Draw Average MSE across all 10 episodes
        ...
        plt.fill_between(lookahead_steps, np.maximum(0, mean_mses - sem_mses), mean_mses + sem_mses,
                         color="#d62728", alpha=0.2, label="±1 SEM (Standard Error of Mean)")
        ...
        # Also save to latent_ttc_mse_horizon.png for consistency
        plt.savefig(os.path.join(output_dir, "latent_ttc_mse_horizon.png"))
        plt.close()
        print(f"Saved Average MSE Lookahead Plot to {mse_plot_path}")

        # Plot 2: Comprehensive Multi-Metric Horizon Analysis (MSE, RMSE, MAE)
        avg_maes = np.array([np.mean(errs) if len(errs) > 0 else 0.0 for errs in step_maes])
        avg_rmses = np.sqrt(mean_mses)
        
        fig, axes = plt.subplots(1, 3, figsize=(16, 4.8), dpi=250)
        ...
        axes[0].set_title(r"Mean Squared Error ($\mathrm{s}^2$)", fontweight="bold")
        axes[0].set_xlabel("Lookahead Step")
        axes[0].set_ylabel(r"$\mathrm{MSE}\ (\mathrm{s}^2)$")
        axes[0].grid(True, linestyle="--", alpha=0.5)
        
        # Subplot 2: RMSE
        axes[1].plot(lookahead_steps, avg_rmses, marker="s", color="#1f77b4", linewidth=2.2)
        axes[1].set_title("Root Mean Squared Error (s)", fontweight="bold")
        ...
        axes[2].set_title("Mean Absolute Error (s)", fontweight="bold")
        axes[2].set_xlabel("Lookahead Step")
        axes[2].set_ylabel("MAE (seconds)")
        axes[2].grid(True, linestyle="--", alpha=0.5)
        
        fig.suptitle(f"TTC Lookahead Performance Horizon ({num_eps} Episodes Avg, {context_len}-Frame Context)", fontsize=13, fontweight="bold", y=1.02)
        plt.tight_layout()
        
        multi_metric_path = os.path.join(output_dir, "latent_ttc_horizon_metrics.png")
        plt.savefig(multi_metric_path)
        plt.close()
        print(f"Saved Multi-Metric Horizon Plot to {multi_metric_path}")
